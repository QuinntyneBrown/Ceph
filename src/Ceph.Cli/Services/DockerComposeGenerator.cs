...
public class DockerComposeGenerator
{
    ...
    private string WriteDockerCompose(GenerateOptions options)
    {
        ...
        if (options.IncludeRgw)
        {
            ...
            sb.AppendLine("        condition: service_healthy");
            sb.AppendLine("    healthcheck:");
            sb.AppendLine("      test: [\"CMD\", \"bash\", \"-c\", \"echo > /dev/tcp/localhost/7480\"]");
            sb.AppendLine("      interval: 15s");
            sb.AppendLine("      timeout: 5s");
            sb.AppendLine("      retries: 10");
            sb.AppendLine("      start_period: 30s");
            sb.AppendLine("    networks:");
            sb.AppendLine("      ceph-net:");
    ...
    private string WriteCephConf(GenerateOptions options)
    {
        ...
        var sb = new System.Text.StringBuilder();
        sb.AppendLine("# Generated by ceph-cli");
        sb.AppendLine("[global]");
        sb.AppendLine($"fsid = {Guid.NewGuid()}");
        sb.AppendLine($"mon_initial_members = {monHosts}");
        sb.AppendLine($"mon_host = {monIps}");
        sb.AppendLine("auth_cluster_required = cephx");
        sb.AppendLine("auth_service_required = cephx");
        sb.AppendLine("auth_client_required = cephx");
        sb.AppendLine($"osd_pool_default_size = {Math.Min(options.OsdCount, 3)}");
        sb.AppendLine("osd_pool_default_min_size = 1");
        sb.AppendLine("osd_journal_size = 1024");
        sb.AppendLine();
        sb.AppendLine("[osd]");
        sb.AppendLine("osd_max_object_name_len = 256");
        sb.AppendLine("osd_max_object_namespace_len = 64");
        sb.AppendLine();
        if (options.IncludeRgw)
        {
            sb.AppendLine("[client.rgw.ceph-rgw]");
            sb.AppendLine("rgw frontends = beast port=7480");
            sb.AppendLine();
        }
...
case "${CEPH_DAEMON}" in
  ...
  OSD_DIRECTORY)
    ...
    if [ ! -f /var/lib/ceph/.osd_id ]; then
      ...
      # Ensure the host bucket is placed under root default so PGs can map
      ceph osd crush move "${OSD_NAME}" root=default 2>/dev/null || true
  ...
  RGW)
    ...
    # Ensure RGW frontend port is configured in the config store
    ceph config set "client.rgw.${RGW_NAME}" rgw_frontends "beast port=7480" 2>/dev/null || true

    echo "=== Starting RGW ${RGW_NAME} ==="
    radosgw -f --name "client.rgw.${RGW_NAME}" --setuser ceph --setgroup ceph &
    RGW_PID=$!

    # Wait for RGW HTTP endpoint to be ready
    echo "=== Waiting for RGW to accept connections on port 7480 ==="
    for i in $(seq 1 60); do
      if bash -c "echo > /dev/tcp/localhost/7480" 2>/dev/null; then
        break
      fi
      sleep 2
    done

    # Create demo S3 user if configured and not already present
    if [ -n "${CEPH_DEMO_UID}" ]; then
      if ! radosgw-admin user info --uid="${CEPH_DEMO_UID}" 2>/dev/null; then
        echo "=== Creating demo S3 user: ${CEPH_DEMO_UID} ==="
